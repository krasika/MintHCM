...
class SugarBean {
   ...
   /**
    * View Tools validation error message array
    */
   public $vt_validation_error_array = [];
   /**
    * View Tools save even if validation returned false
    */
   public $skip_vt_validation = false;
   /**
   ...
   public function __construct() {
      ...
      if ( (!$this->disable_vardefs && empty($loaded_definitions[$this->object_name])) || !empty($GLOBALS['reload_vardefs']) ) {
         ...
         if ( isset($GLOBALS['dictionary'][$this->object_name]) && !$this->disable_vardefs ) {
            $this->field_name_map = $dictionary[$this->object_name]['fields'] ?? null;
...
// View Tools #51928 END
   ...
   public static function get_union_related_list(
           $parentbean, $order_by = '', $sort_order = '', $where = '', $row_offset = 0, $limit = -1, $max = -1, $show_deleted = 0, $subpanel_def = null
   ) {
         ...
         if ( $this_subpanel->isDatasourceFunction() && empty($this_subpanel->_instance_properties['generate_select']) ) {
            ...
            if ( !$first ) {
               $final_query_rows .= ' UNION ALL ( '
                       . $parentbean->create_list_count_query($tmp_final_query) . ' )';
               $final_query .= ' UNION ALL ( ' . $tmp_final_query . ' )';
            } else {
               $final_query_rows = '(' . $parentbean->create_list_count_query($tmp_final_query) . ')';
      ...
      if ( method_exists($parentbean, 'process_union_list_query') ) {
         return $parentbean->process_union_list_query(
                         $parentbean, $final_query, $row_offset, $limit, $max, '', $subpanel_def, $final_query_rows, $secondary_queries
         );
      } else {
   ...
   protected static function build_sub_queries_for_union($subpanel_list, $subpanel_def, $parentbean, $order_by) {
      ...
      foreach ( $subpanel_list as $this_subpanel ) {
         ...
         if (
            ...
            if ( !$this_subpanel->isDatasourceFunction() || ($this_subpanel->isDatasourceFunction() && isset($this_subpanel->_instance_properties['generate_select']) && $this_subpanel->_instance_properties['generate_select'])
            ) {
               ...
               $subquery = method_exists($submodule, 'create_new_list_query') ? $submodule->create_new_list_query(
                               '', $subwhere, $list_fields, $params, 0, '', true, $parentbean, $singleSelect
                       ) : null;
   ...
   public function process_union_list_query(
           $parent_bean, $query, $row_offset, $limit = -1, $max_per_page = -1, $where = '', $subpanel_def = null, $query_row_count = '', $secondary_queries = []
   ) {
      $current_bean = null;
      if ( is_null($subpanel_def) ) {
         $GLOBALS['log']->fatal('subpanel_def is null');
      ...
      if ( $performSecondQuery ) {
            ...
            if ( ($index < $row_offset + $max_per_page || $max_per_page == -99 ) ) {
               ...
               if ( !empty($current_bean->parent_type) && !empty($current_bean->parent_id) ) {
                  if ( !isset($post_retrieve[$current_bean->parent_type]) ) {
                     $post_retrieve[$current_bean->parent_type] = [];
                  }
                  $post_retrieve[$current_bean->parent_type][] = ['child_id' => $current_bean->id, 'parent_id' => $current_bean->parent_id, 'parent_type' => $current_bean->parent_type, 'type' => 'parent'];
               }
         ...
         if ( !empty($sugar_config['disable_count_query']) && !empty($limit) ) {
            ...
            $rows_found = $index ?? $row_offset + count($list);
   ...
   public function _get_num_rows_in_query($query, $is_count_query = false) {
      $num_rows_in_query = 0;
      if ( !$is_count_query ) {
         $count_query = (new SugarBean())->create_list_count_query($query);
      } else {
         $count_query = $query;
      }
               ...
               if ( isset($child_info['parent_type']) && empty($queries[$child_info['parent_type']]) ) {
                  ...
                  if ( isset($field_def['db_concat_fields']) ) {
                     $queries[$child_info['parent_type']] .= ' , '
                             . $this->db->concat(
                                     $templates[$child_info['parent_type']]->table_name, $field_def['db_concat_fields']
                             ) . ' parent_name';
                  } else {
                  ...
                  if ( isset($templates[$child_info['parent_type']]) ) {
                     ...
                     $queries[$child_info['parent_type']] .= " FROM " . $templates[$child_info['parent_type']]->table_name .
   ...
   public function unPopulateDefaultValues() {
      ...
      foreach ( $this->field_defs as $field => $value ) {
         if ( !empty($this->$field) && ((isset($value['default']) && $this->$field == $value['default']) || (!empty($value['display_default']) && $this->$field == $value['display_default']))
         ) {
            $this->$field = null;
            continue;
         }
         if (
                 ...
                 in_array($value['type'], ['date', 'datetime', 'datetimecombo']) &&
               ...
               if ( !class_exists($class) ) {
                  ...
                  if ( !isset($fieldDefs[$rel_name]['relationship']) ) {
                     ...
                     $this->$rel_name = new $class(
                             $fieldDefs[$rel_name]['relationship'], $this, $fieldDefs[$rel_name]
                     );
                  }
               }
            }
   ...
   public function get_linked_beans(
           $field_name, $bean_name = '', $order_by = '', $begin_index = 0, $end_index = -1, $deleted = 0, $optional_where = ""
   ) {
      ...
      if ( $this->load_relationship($field_name) ) {
         if ( $this->$field_name instanceof Link ) {
            ...
            return array_values($this->$field_name->getBeans(
                            new $bean_name(), $order_by, $begin_index, $end_index, $deleted, $optional_where
            ));
         } else {
            // Link2 style
            if ( $end_index != -1 || !empty($deleted) || !empty($optional_where) || !empty($order_by) ) {
               return array_values($this->$field_name->getBeans(['where' => $optional_where, 'deleted' => $deleted, 'limit' => ($end_index - $begin_index), 'order_by' => $order_by]));
            } else {
               return array_values($this->$field_name->getBeans());
            }
         }
   ...
   public function get_importable_fields() {
      ...
      if ( !empty($fieldDefs) ) {
         foreach ( $fieldDefs as $key => $value_array ) {
            if ( (isset($value_array['importable']) && (is_string($value_array['importable']) && $value_array['importable'] == 'false' || is_bool($value_array['importable']) && !$value_array['importable'])) || (isset($value_array['type']) && $value_array['type'] == 'link') || (isset($value_array['auto_increment']) && ($value_array['type'] || $value_array['type'] == 'true'))
            ) {
               // only allow import if we force it
               if ( isset($value_array['importable']) && (is_string($value_array['importable']) && $value_array['importable'] == 'true' || is_bool($value_array['importable']) && $value_array['importable'])
               ) {
   ...
   public function save($check_notify = false) {
      ...
      // View Tools start ev formula logic before hooks
      SugarAutoLoader::requireWithCustom('include/ViewTools/VTFormulaInterpreter.php');
      $Interpreter = new VTFormulaInterpreter();
      $Interpreter->calculateFields($this);
      $Interpreter->validateFields($this);
      // View Tools end
            ...
            if ( isset($this->emailAddress) ) {
               if ( $this->emailAddress instanceof EmailAddress ) {
                  ...
                  $this->emailAddress->saveEmail(
                          $this->id, $this->module_dir, '', '', '', '', '', $this->in_workflow);
               } else {
   ...
   public function fixUpFormatting() {
      global $timedate;
      static $bool_false_values = ['off', 'false', '0', 'no'];
   ...
   public function save_relationship_changes($is_update, $exclude = []) {
      [$new_rel_id, $new_rel_link] = $this->set_relationship_info($exclude);
    ...
    */
   protected function set_relationship_info($exclude = []) {
      $new_rel_id = false;
      $new_rel_link = false;
      // check incoming data
      if ( isset($this->not_use_rel_in_req) && $this->not_use_rel_in_req ) {
         ...
         $rel_id = $this->new_rel_id ?? '';
         $rel_link = $this->new_rel_relname ?? '';
      } else {
         // if we should use relation data from REQUEST
         $rel_id = $_REQUEST['relate_id'] ?? '';
      ...
      return [$new_rel_id, $new_rel_link];
   }
    ...
    */
   protected function handle_preset_relationships($new_rel_id, $new_rel_link, $exclude = []) {
                  ...
                  if ( $this->load_relationship($linkField) ) {
                     ...
                     if ( !empty($this->rel_fields_before_value[$idName]) && empty($this->$idName) ) {
                        ...
                        $success = $this->$linkField->delete(
                                $this->id, $this->rel_fields_before_value[$idName]
                        );
    ...
    * @param array $exclude
    */
   protected function update_parent_relationships($exclude = []) {
      if ( !is_array($this->field_defs) && !is_object($this->field_defs) ) {
                  ...
                  if ( !empty($this->fetched_row[$typeField]) && !empty($this->fetched_row[$idField]) && !empty($parentLinks[$this->fetched_row[$typeField]]) && ($this->fetched_row[$idField] != $this->$idField)
                     ...
                     if ( $this->load_relationship($oldParentLink) ) {
                        ...
                        SugarRelationship::addToResaveList(BeanFactory::getBean(
                                        $this->fetched_row[$typeField], $this->fetched_row[$idField]
                        ));
                     }
                  }
         ...
         if ( $this->load_relationship($new_rel_link) ) {
            ...
            if ( $this->load_relationship($lower_link) ) {
               ...
               $rel = Relationship::retrieve_by_modules(
                               $new_rel_link, $this->module_dir, $this->db, 'many-to-many'
               );
   ...
   public function hasEmails() {
      return (!empty($this->field_defs['email_addresses']) && $this->field_defs['email_addresses']['type'] == 'link' && !empty($this->field_defs['email_addresses_non_primary']) && $this->field_defs['email_addresses_non_primary']['type'] == 'email'
              );
   }
   ...
   private function _sendNotifications($check_notify) {
      if ( $check_notify || (isset($this->notify_inworkflow) && $this->notify_inworkflow) && !$this->isOwner($this->created_by)
      ) {
   ...
   public function get_list(
           $order_by = "", $where = "", $row_offset = 0, $limit = -1, $max = -1, $show_deleted = 0, $singleSelect = false, $select_fields = []
   ) {
      ...
      $query = $this->create_new_list_query(
              $order_by, $where, $select_fields, [], $show_deleted, '', false, null, $singleSelect
      );
   ...
   public function getOwnerWhere($user_id) {
      ...
      if ( isset($this->field_defs['assigned_user_id']) ) {
         ...
         if(is_array($subordinates_ids) || $subordinates_ids instanceof \Countable ? count($subordinates_ids) : 0){
   ...
   public function create_new_list_query(
           $order_by, $where, $filter = [], $params = [], $show_deleted = 0, $join_type = '', $return_array = false, $parentbean = null, $singleSelect = false, $ifListForExport = false
   ) {
      ...
      if ( $this->module_dir == 'Users' && !is_admin($current_user) && isset($sugar_config['securitysuite_filter_user_list']) && $sugar_config['securitysuite_filter_user_list']
      ) {
         require_once('modules/SecurityGroups/SecurityGroup.php');
         global $current_user;
     ...
     if((!empty($current_view) && $current_view->type=='list') || $action=="SubPanelViewer" || $dynamic_action=='displayDashlet' || $entry_point=="retrieve_dash_page" ){
      ...
      foreach ( $fields as $field => $value ) {
         ...
         if ( isset($data['type']) && $data['type'] == 'parent' ) {
            ...
            $match = preg_match(
                    '/(^|[\s(])parent_([a-zA-Z]+_?[a-zA-Z]+)_([a-zA-Z]+_?[a-zA-Z]+)\.name/', $where, $matches
            );
            if ( $match ) {
               $joinTableAlias = 'jt' . $jtcount;
               ...
               //Replace any references to the relationship in the where clause with the new alias
               $where = preg_replace(
                       '/(^|[\s(])parent_' . $joinModule . '_' . $joinTable . '\.name/', '${1}' . $nameField, $where
               );
               $jtcount++;
            }
         }
            ...
            if ( !empty($this->$linkField) ) {
               ...
               if ( isset($data['rname']) && $data['rname'] == 'name' && !empty($beanFiles[$beanList[$rel_module]]) ) {
                  ...
                  if ( isset($rel_mod->field_name_map['first_name']) && isset($rel_mod->field_name_map['last_name']) ) {
                     $data['db_concat_fields'] = [0 => 'first_name', 1 => 'last_name'];
                  }
               }
               ...
               if ( isset($data['db_concat_fields']) ) {
                  ...
                  if ( in_array('first_name', $data['db_concat_fields']) && in_array('last_name', $data['db_concat_fields']) ) {
                     ...
                     if ( preg_match($exp, $where, $matches) ) {
                        ...
                        $first_name_search = str_replace(
                                $data['name'], $params['join_table_alias'] . '.first_name', $search_expression
                        );
                        $last_name_search = str_replace(
                                $data['name'], $params['join_table_alias'] . '.last_name', $search_expression
                        );
                        $full_name_search = str_replace(
                                $data['name'], $this->db->concat($params['join_table_alias'], $data['db_concat_fields']), $search_expression
                        );
                        $buildWhere = true;
                        $where = str_replace(
                                $search_expression, '(' . $full_name_search . ' OR ' . $first_name_search
                                . ' OR ' . $last_name_search . ')', $where
                        );
                     }
                  }
                  ...
                  $where = preg_replace(
                          '/(^|[\s(])' . $data['name'] . '/', '${1}' . $params['join_table_alias'] . '.' . $data['rname'], $where
                  );
                  ...
                  if ( isset($data['relationship_fields']) ) {
                     foreach ( $data['relationship_fields'] as $r_name => $alias_name ) {
                        $where = preg_replace(
                                '/(^|[\s(])' . $alias_name . '/', '${1}' . $params['join_table_link_alias'] . '.' . $r_name, $where
                        );
                     }
                  }
               }
      ...
      if (strpos($ret_array['where'], $group_where) !== false && !in_array($this->module_name, $skipped_modules) ) { //Mint #60146
         ...
         foreach ( ['from', 'from_min', 'secondary_from'] as $eVSecGroupUpdKey ) {
   ...
   public function get_detail(
           $order_by = "", $where = "", $offset = 0, $row_offset = 0, $limit = -1, $max = -1, $show_deleted = 0
   ) {
      ...
      /* END - SECURITY GROUPS */
      $query = $this->create_new_list_query($order_by, $where, [], [], $show_deleted, $offset);
   ...
   public function convertField($fieldValue, $fieldDef) {
      if ( !empty($fieldValue) && !(isset($fieldDef['source']) && !in_array($fieldDef['source'], ['db', 'custom_fields', 'relate']) && !isset($fieldDef['dbType']))
      ) {
      ...
      if ( property_exists($this, 'currency_id') && $this->currency_id == -99 ) {
         ...
         if ( $currency ) {
            ...
            foreach ( $this->field_defs as $this_field ) {
               if ( isset($this_field['type']) && $this_field['type'] == 'relate' && isset($this_field['module']) && $this_field['module'] == 'Currencies' && isset($this_field['id_name']) && $this_field['id_name'] == 'currency_id'
               ) {
   ...
   public function fill_in_additional_parent_fields() {
      ...
      if ( !empty($this->parent_type) ) {
         ...
         $this->getRelatedFields($this->parent_type, $this->parent_id, ['name' => 'parent_name', 'document_name' => 'parent_document_name', 'first_name' => 'parent_first_name', 'last_name' => 'parent_last_name']
         );
         if ( !empty($this->parent_first_name) || !empty($this->parent_last_name) ) {
            $this->parent_name = $GLOBALS['locale']->getLocaleFormattedName(
                    $this->parent_first_name, $this->parent_last_name
            );
      ...
      foreach ( $fields as $field => $alias ) {
         if ( !empty($GLOBALS['dictionary'][$object]['fields'][$field]['db_concat_fields']) ) {
            $concat = $this->db->concat(
                    $table, $GLOBALS['dictionary'][$object]['fields'][$field]['db_concat_fields']
            );
   ...
   public function get_related_list(
           $child_seed, $related_field_name, $order_by = "", $where = "", $row_offset = 0, $limit = -1, $max = -1
   ) {
      global $layout_edit_mode;
      ...
      if ( $this->$related_field_name instanceof Link ) {
         ...
         $query_array = $this->$related_field_name->getQuery(["return_as_array" => true, 'where' => '1=1']);
      }
   ...
   protected function restoreFiles() {
      ...
      foreach ( $files as $file ) {
         if ( is_file('upload://deleted/' . $directory . '/' . $file) && !sugar_rename('upload://deleted/' . $directory . '/' . $file, 'upload://' . $file)
         ) {
   ...
   public function haveFiles() {
      $return = false;
      if ( $this->bean_implements('FILE') || $this instanceof File || !empty(static::$fileFields[$this->module_name])
      ) {
         $return = true;
   ...
   public function deleteFiles() {
      ...
      foreach ( $files as $file ) {
         if ( file_exists('upload://' . $file) && !sugar_rename('upload://' . $file, 'upload://deleted/' . $directory . '/' . $file)
         ) {
       ...
       */
      $db = DBManagerFactory::getInstance();
      $record = ['bean_id' => $db->quoted($this->id), 'module' => $db->quoted($this->module_name), 'date_modified' => $db->convert($db->quoted(date('Y-m-d H:i:s')), 'datetime')];
   ...
   public function build_related_list_where(
           $query, &$template, $where = '', $in = '', $order_by = '', $limit = '', $row_offset = 0
   ) {
      $db = DBManagerFactory::getInstance('listviews');
   ...
   public function set_relationship(
           $table, $relate_values, $check_duplicates = true, $do_update = false, $data_values = null
   ) {
      $where = '';
   ...
   public function create_export_query($order_by, $where) {
      return $this->create_new_list_query($order_by, $where, [], [], 0, '', false, $this, true, true);
   }
