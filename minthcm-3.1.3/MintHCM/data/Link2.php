...
class Link2 {
      ...
      if ( empty($bean->field_defs) || empty($bean->field_defs[$linkName]) || empty($bean->field_defs[$linkName]['relationship']) ) {
         if ( empty($linkDef) ) {
            ...
            $this->def = VardefManager::getLinkFieldForRelationship(
                            $bean->module_dir, $bean->object_name, $linkName
            );
         } else {
            $this->def = $linkDef;
         }
   ...
   /**
    * Function added by View Tools
    * @return type
    */
   public function getOppositeModuleNameAndKey() {
      if ( $this->getSide() == REL_LHS ) {
         $join_side = "rhs";
         $side = "lhs";
      } else {
      ...
      $module = $this->relationship->def[$join_side . '_module'];
      if ( isset($this->relationship->def['join_key_' . $join_side]) ) {
         $key = $this->relationship->def['join_key_' . $join_side];
      } else {
