...
/**
 * Mint #52883
 * Added is_locked property, which allows outer object to lock dashlet (remove "Edit" and "Remove" actions from header)
 * Modified getHeader method to be more optimal (removed old code and added is_locked proprety assign to Smarty)
 * File from SuiteCRM 7.10.7
 */
...
//   public function getHeader($text = '') {
    ...
    public function loadLanguage($dashletClassname, $dashletDirectory = 'modules/Home/Dashlets/')
    {
        ...
        if (!isset($dashletStrings[$dashletClassname])) {
            ...
            if (is_file($dashletDirectory . $dashletClassname . '/' . $dashletClassname . '.' . $current_language . '.lang.php') || is_file('custom/' . $dashletDirectory . $dashletClassname . '/' . $dashletClassname . '.' . $current_language . '.lang.php')) {
