...
class EditView {
   ...
   $module, $focus = null, $metadataFile = null, $tpl = 'include/EditView/EditView.tpl', $createFocus = true, $metadataFileName = 'editviewdefs'
   ) {
      ...
      if ( !empty($this->metadataFile) && file_exists($this->metadataFile) ) {
         ...
         if ( !file_exists("modules/$this->module/metadata/$metadataFileName.php") && file_exists("modules/$this->module/EditView.html")
         ) {
   ...
   public function process($checkFormName = false, $formName = '') {
      ...
      if ( $this->showVCRControl ) {
         $this->th->ss->assign(
                 'PAGINATION', SugarVCR::menu(
                         $this->module, $this->offset, $this->focus->is_AuditEnabled(), $this->view === 'EditView'
                 )
         );
      ...
      if ( $this->focus ) {
         ...
         //viewTools ViewTools begin ref #34457 [7.7.X] Ukrywanie pól na widoku szczegółowym po stronie PHP
         require_once 'include/ViewTools/VTFormulaInterpreter.php';
         $view_tools = new VTFormulaInterpreter();
         $fields_to_hide = $view_tools->getToHideFields($this->focus);
         //viewTools ViewTools end
         ...
         foreach ( $this->focus->toArray() as $name => $value ) {
            ...
            if ( $this->focus->bean_implements('ACL') ) {
               if ( isset($fields_to_hide[$name]) ) {
                  $this->fieldDefs[$name]['acl'] = 0;
               } else {
                  $this->fieldDefs[$name]['acl'] = 4; //user has full access to field. Look on SugarPro-6.X.X
               }
            } else {
               $this->fieldDefs[$name]['acl'] = 4; //user has full access to field
            }
            //viewTools ViewTools end
   ...
   public function display($showTitle = true, $ajaxSave = false) {
      ...
      if ( isset($this->defs['templateMeta']['form']['closeFormBeforeCustomButtons']) ) {
         $this->th->ss->assign(
                 'closeFormBeforeCustomButtons', $this->defs['templateMeta']['form']['closeFormBeforeCustomButtons']
         );
      }
      ...
      $str .= $this->th->displayTemplate(
              $this->module, $form_name, $this->tpl, $ajaxSave, $this->defs
      );
   ...
   public function getValueFromRequest($request, $name) {
      ...
      if ( preg_match('/^date_(.*)$/s', $name, $matches) ) {
         ...
         if ( isset($request[$timeKey]) ) {
            ...
            if ( isset($request[$timeHourKey]) && isset($request[$timeMinuteKey])
            ) {
               $d .= sprintf(
                       ' %s:%s', $request[$timeHourKey], $request[$timeMinuteKey]
               );
            }
