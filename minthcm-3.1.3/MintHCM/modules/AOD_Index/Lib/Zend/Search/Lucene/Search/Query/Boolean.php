...
class Zend_Search_Lucene_Search_Query_Boolean extends Zend_Search_Lucene_Search_Query
{
    ...
    private function _calculateConjunctionResult()
    {
        ...
        foreach ($this->_subqueries as $subqueryId => $subquery) {
            ...
            $resVectorsSizes[] = is_array(end($resVectors)) || end($resVectors) instanceof \Countable ? count(end($resVectors)) : 0;
        ...
        foreach ($resVectors as $nextResVector) {
            ...
            if ((is_array($this->_resVector) || $this->_resVector instanceof \Countable ? count($this->_resVector) : 0) == 0) {
    ...
    private function _calculateNonConjunctionResult()
    {
        ...
        foreach ($this->_subqueries as $subqueryId => $subquery) {
            if ($this->_signs[$subqueryId] === true) {
                ...
                $requiredVectorsSizes[] = is_array(end($requiredVectors)) || end($requiredVectors) instanceof \Countable ? count(end($requiredVectors)) : 0;
        ...
        foreach ($requiredVectors as $nextResVector) {
            ...
            if ((is_array($required) || $required instanceof \Countable ? count($required) : 0) == 0) {
                // Empty result set, we don't need to check other terms
                break;
            }
        }
